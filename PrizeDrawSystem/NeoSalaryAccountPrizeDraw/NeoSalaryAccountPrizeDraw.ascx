...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="NeoSalaryAccountPrizeDraw.ascx.cs" Inherits="PrizeDrawSystem.NeoSalaryAccountPrizeDraw.NeoSalaryAccountPrizeDraw" %>


<div class="row">
    <div class="col-md-12">
        <h2 class="text-center mt-5">
            <%=DrawType %> Draw (New Salary Account) 
            <br />
            السحب لمرة واحدة (السحب على الفائزين في عرض تحويل الراتب)
        </h2>
        <hr class="mb-5">
        ...
        <div id="winnersDiv" style="display: none;">
            <h2 class="winnersHeading">Winners
            </h2>
...
<script type="text/javascript">
    ...
    function LoadConfigurations() {
        ...
        $.ajax({
            type: "POST",
            url: methodUrl,
            data: JSON.stringify({ drawCategory: "New Salary Account" }),
    ...
    function LoadDrawConfigurations() {
        ...
        var methodUrl = [location.protocol, '//', location.host].join('') + '/_layouts/15/PrizeDrawSystem/WebAPI.aspx/GetMaxNewSalaryAccountWinners';
        $.ajax({
            type: "POST",
            url: methodUrl,
            data: JSON.stringify({}),
    ...
    function DeleteAndArchiveData() {
        ...
        var methodUrl = [location.protocol, '//', location.host].join('') + '/_layouts/15/PrizeDrawSystem/WebAPI.aspx/ArchiveAndDeleteNewSalaryAccountData';
        $.ajax({
            type: "POST",
            url: methodUrl,
            data: JSON.stringify({}),
    ...
    function SaveWinnerResults() {
        ...
        if (winners != null && winners.length > 0) {
            ...
            for (var i = 0; i < winners.length; i++) {
                ...
                winners[i].AccountOpeningDate = new Date(parseInt(winners[i].AccountOpeningDate.substr(6))).toJSON();
                winners[i].LastSalaryTransferDate = new Date(parseInt(winners[i].LastSalaryTransferDate.substr(6))).toJSON();
                delete winners[i].__type;
            }

            $('.ajax-loader').css("visibility", "visible");

            var methodUrl = [location.protocol, '//', location.host].join('') + '/_layouts/15/PrizeDrawSystem/WebAPI.aspx/SaveNewSalaryAccountWinners';
            $.ajax({
                type: "POST",
                url: methodUrl,
    ...
    $(function () {
        ...
        $('.startDrawBtn').click(function () {
            ...
            if (maxWinners > 5) {
                ...
                methodUrl += "GetNewSalaryAccountAllWinners";
