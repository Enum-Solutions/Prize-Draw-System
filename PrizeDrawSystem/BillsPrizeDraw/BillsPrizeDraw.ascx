...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="BillsPrizeDraw.ascx.cs" Inherits="PrizeDrawSystem.BillsPrizeDraw.BillsPrizeDraw" %>


<div class="row">
    <div class="col-md-12">
        <h2 class="text-center mt-5">
            <%=DrawType %> Draw (Utility bills)
            <br />
            السحب لمرة واحدة (السحب على الفائزين في عرض دفع الفواتير عبر تطبيق بنك نزوى)
        </h2>
        <hr class="mb-5">
...
<script type="text/javascript">
    ...
    function LoadDrawConfigurations() {
        ...
        var methodUrl = [location.protocol, '//', location.host].join('') + '/_layouts/15/PrizeDrawSystem/WebAPI.aspx/GetMaxBillsWinners';
        $.ajax({
            type: "POST",
            url: methodUrl,
            data: JSON.stringify({}),
    ...
    function DeleteAndArchiveData() {
        ...
        var methodUrl = [location.protocol, '//', location.host].join('') + '/_layouts/15/PrizeDrawSystem/WebAPI.aspx/ArchiveAndDeleteBillsData';
        $.ajax({
            type: "POST",
            url: methodUrl,
            data: JSON.stringify({}),
    ...
    function SaveWinnerResults() {
        ...
        if (winners != null && winners.length > 0) {
            ...
            var methodUrl = [location.protocol, '//', location.host].join('') + '/_layouts/15/PrizeDrawSystem/WebAPI.aspx/SaveBillsWinners';
            $.ajax({
                type: "POST",
                url: methodUrl,
    ...
    $(function () {
        ...
        $('.startDrawBtn').click(function () {
            ...
            if (maxWinners > 5) {
                ...
                methodUrl += "GetBillsSingleWinner";
